...
from .utils import checkFsAlign
...
class FileReader():
    """ Read multiple samples from the FFReocrd file
    ...
    aio_fd:       
        opened file descriptor for aio read
    ...
    offsets:      
        offsets of each samples
    
    checksums:    
        checksums of each samples
    
    checksum_all: 
        checksum of all data (disabled)
    """
    ...
    def __init__(self,
        ...
        ----------
            filenames:      
                file to be opened
            check_data: 
                validate checksum or not
        """
        if not isinstance(filename, (str, os.PathLike)):
            raise TypeError("{} must be initalised with a filename to open".format(self.__class__.__name__))
        self.fd = os.open(filename, os.O_RDONLY)
        
        if checkFsAlign(self.fd):
            self.aio_fd = os.open(filename, os.O_RDONLY | os.O_DIRECT)
        else:
